...
#define FILENUMPKS 15
  ...
  /*mc*/
  /* info for undo feature data */
  int oldlox, oldnumx;
  /* info for shared y-axis feature */
  int sharedyaxis;
  int sharedlocnt, sharedhicnt, sharedncnts;
  /* info for matrix features */
#define MAX_MATRIX_DIM 16384
  char matfilnam[80];
  float projx[MAX_MATRIX_DIM], projy[MAX_MATRIX_DIM];
  int maxx, maxy;
  int matitype, matswab;
  unsigned long matoffset;
  float randk;
  int randsinglesmaxx, randsinglesmaxy;
  float randsinglesx[MAX_MATRIX_DIM], randsinglesy[MAX_MATRIX_DIM];
  /*mc */
...
/* mc */
/* prototypes for new functions called before their definition */
int set_matrix_dim(char *paramstr);
int set_matrix(char *filnam, int nc);
void gate(char);
void limit_display();
int findsharedlohi (int, int, int, int);
int writeall(int *, char *, int);
int readall(int *, char *, int);
void display_gf3m_help();
int hcopy_noninteractive(char* ans);
/* mc */
...
int addspec(char *command, int nc)
{
  ...
  float fact, rj1, rj2, save[16384];
  int   i, numch;
  char  namesp2[8], ans[80];
...
int dspsp(int in1, int in2, int in3)
{
  ...
  int   i1, icol, i, nx, ny, ncc, ich, loy;
  char  heading[28];
  ...
  /*mc force shared y axis -- override above display settings */
  if (gf3gd.sharedyaxis == 1)
    {
      x0 = (float) gf3gd.loch;
      dx = (float) gf3gd.nchs;
      /* use y0 specified by user with Y0 command */
      y0 = (float) gf3gd.sharedlocnt;        /* alternative would be: y0 = (float) gf3gd.sharedlocnt; */
      /* in ncts = 0 (as from NY 0), set common hi cnts, or else use ncts specified by user with NY command  */
      if (gf3gd.sharedncnts <= 0)
        dy = gf3gd.sharedhicnt - (y0 - 1);
      else
        dy = (float) gf3gd.sharedncnts + 1;
    }
  ...
  /*mc*/
  /*mc draw line at y=0 */
  /* done above: initg(&nx, &ny);*/
  /* must come after trax defines xy coordinate frame */
  setcolor(11);  /* 10 = cadet blue ... why must add 1???  see utils/minig/miniga.c */
  pspot(x0,0.);
  vect(x0+dx,0.);
  /* refresh x-axis frame in black in case overwritten */
  setcolor(1);
  pspot(x0,y0);
  vect(x0+dx,y0);
...
int gfexec(char *ans, int nc)
{
  ...
  if (!strncmp(ans, "AC", 2) || !strncmp(ans, "AS", 2) || 
    ...
    /*mc suppress initial display in shared y-axis mode, to wait for properly-scaled redisplay */
    if (! (gf3gd.sharedyaxis == 1) )
      /*mc*/
...
#undef W
      ...
      /*mc now redisplay with shared y-axis if necessary */
      if (gf3gd.sharedyaxis == 1)
        goto SHRDSP;
      /*mc*/
    ...
    lookup(ans, nc);
  } else if ((!strncmp(ans, "MA", 2)) /*mc*/ && strncmp(ans, "MAtr", 4) /*mc*/) {
    ...
    for (i = isplo; i <= isphi; ++i) {
      colormap[1] = 2 + ((i-isplo)%14);
      sprintf(ans, "SP %d", i);
      if (getsp(ans, strlen(ans))) break;
      if (dspsp(1, 1, in2)) goto BADCMD;
    }
    colormap[1] = j;
    gf3gd.disp = 1;
  ...
  /*mc  ADD NEW COMMANDS HERE */
  
  else if (!strncmp(ans, "BA", 2)) {
    /* shift display to previous position */
    if (gf3gd.disp) {
      if (inin(ans + 2, nc - 2, &idata, &in, &in2)) goto BADCMD;
      gf3gd.lox = gf3gd.oldlox;
      gf3gd.numx = gf3gd.oldnumx;
      goto SHRDSP;
    }
    printf("Bad command: New spectrum not yet displayed...\n");
  } else if (!strncmp(ans, "GC", 2)) {
    /* shift display centered on given channel */
    if (gf3gd.disp) {
      if (inin(ans + 2, nc - 2, &idata, &in, &in2)) goto BADCMD;
      gf3gd.oldlox = gf3gd.lox;
      gf3gd.oldnumx = gf3gd.numx;
      gf3gd.lox = idata - gf3gd.numx / 2;

      goto SHRDSP;
    }
    printf("Bad command: New spectrum not yet displayed...\n");
  } else if (!strncmp(ans, "GE", 2)) {
    /* shift display centered on given energy */
    if (gf3gd.disp) {
      if (gf3gd.ical != 1) {
        printf ("GE -- Warning: Calibration not defined\n");
        goto BADCMD;
      }
      if (gf3gd.nterms != 2) printf ("GE -- Warning: Calibration terms above linear ignored\n");
      gf3gd.oldlox = gf3gd.lox;
      gf3gd.oldnumx = gf3gd.numx;
      if (sscanf(ans+2,"%f",&x) != 1) {
        printf ("ERROR: Nonnumeric parameter for energy.\n");
        goto BADCMD;
      }
      gf3gd.lox = (x - gf3gd.gain[0]) / gf3gd.gain[1] - gf3gd.numx / 2;
    ...
    /* shift left */
    /*mc*/
    gf3gd.ncnts /= 2;  /* only works if already NY nonzero */
    /*mc*/
    ...
    if (idata == 1)
      {      
        gf3gd.sharedyaxis = 1;
        printf ("Shared y-axis mode ON.\n");
      }
    else if (idata == 0)
      {        
        gf3gd.sharedyaxis = 0;
        printf ("Shared y-axis mode OFF.\n");
        gf3gd.ncnts = 0;  /* restore proper y-axis diplay (NY 0) */
      }

  } else if (!strncmp(ans, "DIm", 3)) {
    if (set_matrix_dim(ans+3))
      goto BADCMD;
  } else if (!strncmp(ans, "MAtr", 4)) {
    /* define matrix and read projections */
    set_matrix(ans,nc);
  } else if (!strncmp(ans, "PRojx", 5)) {
    /* display matrix x projection */
    for (i = 0; i <= gf3gd.maxx; i++) {
      gf3gd.spec[i] =  gf3gd.projx[i];
    }
    ...
    /* gate on y projection */
    gate('y');
  } else if (!strncmp(ans, "RA", 2) && ! !strncmp(ans, "RAn", 3)) {
    /* comparison "RA " necessary to avoid conflict with "RAnd" */
    /* gate on y projection */
    if(!readall(&maxspec, ans, nc))
      gfexec("RD 1",4);
    else
      erase();  /* prevent confusion of user thinking leftover spectrum is new spectrum */
  } else if (!strncmp(ans, "WA", 2)) {
    /* gate on y projection */
    writeall(&maxspec, ans, nc);
  } else if (!strncmp(ans, "RAnd", 4)) {
    /* establish time random subtraction parameters */
    if (sscanf(ans+4,"%f %*s %*s",&x) < 1) 
      {
        printf ("ERROR: Nonnumeric first parameter for RAND.  Use 0 to disable randoms subtraction.\n");
        goto BADCMD;
      }
    gf3gd.randk = x;
    if (x == 0.) 
      {
        printf ("Randoms subtraction disabled.\n");
      }
    else
      {
        printf ("Randoms subtraction enabled.  acceptance constant K = %e\n", gf3gd.randk);

        /* read x singles */
        if (sscanf(ans+4,"%*f %s %*s", &q[0]) < 1)
          {
            printf ("ERROR: No x singles file name.\n");
            goto BADCMD;
          }
        printf ("   Reading x singles spectrum...\n");
        if (readsp(q, gf3gd.randsinglesx, cmd, &numch, 8192))
          {
            printf ("ERROR: x singles file not readable.\n");
            goto BADCMD;
          }
        gf3gd.randsinglesmaxx = numch - 1;
        /* read y singles */
        if (sscanf(ans+4,"%*f %*s %s", &q[0]) < 1)
          {
            printf ("ERROR: No y singles file name.\n");
            goto BADCMD;
          }
        printf ("   Reading y singles spectrum...\n");
        if(readsp(q, gf3gd.randsinglesy, cmd, &numch, 8192))
          {
            printf ("ERROR: y singles file not readable.\n");
            goto BADCMD;
          }
 ...
 SHRDSP:
  /*mc adjust new x-axis limits to be within channels allowed 
    for spectrum by shifting left of right if need be */
  limit_display();
  /*mc*/

  /*mc shared axis display*/
  if (gf3gd.sharedyaxis == 1)
    {
      gf3gd.sharedncnts = gf3gd.ncnts;  /* use current value of ncnts to override individual spectra's values */
      if (gf3gd.sharedncnts <= 0)      
        findsharedlohi(maxspec, idata, in, idata);
      gf3gd.sharedlocnt = gf3gd.locnt;   /* use most recent y-axis low value specified with Y0 command */
    }
  /*mc*/
...
int gfinit(int argc, char **argv)
{
  ...
  if (first) {
    ...
    /*mc set default shared y axis mode to off */
    gf3gd.sharedyaxis = 0;
    /*mc*/
    ...
    /*mc*/
    /* set defualt matrix dimensions */
    printf("\n");
    set_matrix_dim("4096 4096 2 0 0");
    /*mc*/
...
int lookup(char *filnam, int nc)
{
  ...
  winfile = open_new_file(filnam, 0);
...
/*=======================================================================
 * following functions added by mc 
 *=======================================================================*/

void reverse_byte_order (void* buffer, int size)
/* reverses byte order of buffer, assuming buffer to be size bytes long */
{
  int ibyte;
  char tmp;

  for (ibyte = 0; ibyte < size / 2; ibyte++)
    {
      tmp = ((char*) buffer) [ibyte];
      ((char*) buffer) [ibyte] = ((char*) buffer) [size - ibyte - 1];
      ((char*) buffer) [size - ibyte - 1] = tmp;
    }
}

/* read_one_row -- used by matread_slice */
/* returns 0 if success, 1 if failure */
int read_one_row (FILE* file, int* i4row)
{
  short i2row[MAX_MATRIX_DIM];
  char i1row[MAX_MATRIX_DIM];
  int read_code = 0;
  int ix;

  /* read row */
  if (gf3gd.matitype==1)
    read_code=fread(i1row,1,(gf3gd.maxx+1),file);
  else if (gf3gd.matitype==2)
    read_code=fread(i2row,2,(gf3gd.maxx+1),file);
  else if (gf3gd.matitype==4)
    read_code=fread(i4row,4,(gf3gd.maxx+1),file);
  if (read_code!=(gf3gd.maxx+1)) 
    return 1;
  ...
  if (gf3gd.matswab)
    {
      if (gf3gd.matitype==2)
        for (ix = 0; ix <= gf3gd.maxx; ix++) 
          reverse_byte_order(&i2row[ix],2);
      else if (gf3gd.matitype==4)
        for (ix = 0; ix <= gf3gd.maxx; ix++) 
          reverse_byte_order(&i4row[ix],4);
    }
...
/* ======================================================================= */
/* returns 0 if success, 1 if failure */
int matread_slice(char *fn, float *sp, int *numch, 
    int ilo, int ihi, char gateaxis, int addtosp, float coeff)
  /* 
     fn = file name
     sp = returned output spectrum
     numch = returned number of channels in output spectrum
     ilo, ihi = channel range
     gateaxis = axis for gate
     addtosp = whether or not to add values to existing spectrum
     coeff = weight factor
  */
{
  int i4row[MAX_MATRIX_DIM], i4sum[MAX_MATRIX_DIM];
  int  i, iy, ix;
  FILE  *file;
  int read_code;

  /* validate gate channels */
  if (
      (ilo < 0) 
      || ((gateaxis=='x')&&(ihi > gf3gd.maxx)) 
      || ((gateaxis=='y')&&(ihi > gf3gd.maxy))
      )
    {
      printf("Bad %c-gate channel limits for .mat file: %d %d\n", gateaxis, ilo, ihi);
      return 1;
    }

  /* set size of object spectrum */
  if (gateaxis=='x')
    *numch = gf3gd.maxy+1;
  else if (gateaxis=='y')
    *numch = gf3gd.maxx+1;

  printf ("  gate %c, channels %d..%d, weight %f\n", gateaxis,ilo,ihi,coeff);

  /* open file and skip header */
  if (!(file = open_readonly(fn))) 
    {
      return 1;
    }
  fseek(file,gf3gd.matoffset,SEEK_CUR);
  ...
  /* go scan through matrix, grabbing needed values */
  if (gateaxis == 'x') 
    {
      /* read *all* matrix rows into i2mat and add *relevant* entries into i4sum */
      for (iy = 0; iy <= gf3gd.maxy; ++iy) 
        {
          read_code=read_one_row(file,i4row);
          if (read_code)
            {
    ...
    {
      /* read *relevant* matrix rows into i2mat and add *all* columns into i4sum */
      fseek(file, ilo*(gf3gd.maxx+1)*gf3gd.matitype, SEEK_CUR);
      for (iy = ilo; iy <= ihi; iy++) 
        {
          read_code=read_one_row(file,i4row);
          if (read_code)
  ...
  /* convert sum to float and add or store to returned spectrum  */

    for (i = 0; i < *numch; i++) 
      {
        if (addtosp) 
          /* add to existing spectrum */
          sp[i] += coeff * ((float) i4sum[i]);  
        else
          /* store to existing spectrum */
          sp[i] = coeff * ((float) i4sum[i]);   
      }
...
int set_matrix_dim(char *paramstr)
/* returns: 0 if okay values, 1 if not */
{

  int read_code;
  int numx, numy, matitype, matswab;
  unsigned long matoffset;

  /* read matrix dimensions */
  read_code = sscanf(paramstr,"%d %d %d %d %lu ",&numx,&numy,&matitype,&matswab,&matoffset);
  if ( read_code < 4) 
    {
      printf ("ERROR: Must specify at least four integer arguments to dim.\n");
      return 1;
    }
  if (read_code == 4)
    matoffset = 0;
  ...
  /* validate arguments */
  if (!( (1<=numx) && (numx<=MAX_MATRIX_DIM) && (1<=numy) && (numy<=MAX_MATRIX_DIM)))
    {
      printf ("ERROR: Matrix x and y dimensions must be in range 1..%d.\n",MAX_MATRIX_DIM);
      return 1;
    }
  if (!( (matitype==1) || (matitype==2) || (matitype==4) ))
    {
      printf ("ERROR: Only 1, 2, and 4 byte signed integers are supported for matrices.\n");
      return 1;
    }
  if ((matitype==1)&&(sizeof(char)!=1))
    {
      printf ("ERROR: sizeof(char) is not 1 for the compiler used to compile gf3m \n"
              "  on this computer.  gf3m will not properly read 1-byte integers.\n");
      return 1;
    }
  else if ((matitype==2)&&(sizeof(short int)!=2))
    {
      printf ("ERROR: sizeof(short int) is not 2 for the compiler used to compile gf3m \n"
              "  on this computer.  gf3m will not properly read 2-byte integers.\n");
      return 1;
    }
  else if ((matitype==4)&&(sizeof(int)!=4))
    {
      printf ("ERROR: sizeof(int) is not 4 for the compiler used to compile gf3m \n"
              "  on this computer.  gf3m will not properly read 4-byte integers.\n");
      return 1;
    }
  if (!( (matswab==0) || (matswab==1) ))
    {
      printf ("ERROR: Must specify 0 or 1 for byte swapping.\n");
      return 1;
    }
  ...
  gf3gd.maxx = numx - 1;
  gf3gd.maxy = numy - 1;
  gf3gd.matitype = matitype;
  gf3gd.matswab = matswab;
  gf3gd.matoffset = matoffset;
  printf("Setting matrix dimensions: %dx%dx(%d bytes), swapping %s, offset %lu bytes\n",
         numx,numy,matitype,matswab?"ON":"OFF",matoffset);
...
int set_matrix(char *filnam, int nc)
{
  ...
  /* ask for and validate spectrum file name */
  strncpy(filnam, "    ", 4);
  if (nc <= 4 &&
      !cask("Matrix file = ?", filnam, 80)) return 0; 
  setext(filnam, ".mat", 80);
  if (!inq_file(filnam, &jrecl_dummy)) {
    file_error("open", filnam);
    return 1;
  }
  strcpy(gf3gd.matfilnam,filnam);

  /* read projections */
  printf ("Reading projections from matrix file %s...\n", filnam);
  matread_slice(filnam, gf3gd.projx, &numch,  0, gf3gd.maxy, 'y', 0, 1.0);
  matread_slice(filnam, gf3gd.projy, &numch,  0, gf3gd.maxx, 'x', 0, 1.0);
...
void gate(char gateaxis)
{

  int swaptmp;
#define SWAP(A,B) swaptmp = B; B = A; A = swaptmp;
  int i, ix, iy;
  float weight;
  float x,y;
  char ans[80];
  ...
  /* gate parameters */
  int ilo, ihi;
  int bgilo[30], bgihi[30];
  int numbg;
  int totgatech, totbgch;

  /* randoms subtraction */
  int colormapsave;
  int gatesinglescts, bgsinglescts;
  float randmultiplier;

  printf("Set limits for gate; 'A' to abort...\n");
  retic (&x, &y, ans);
  if (ans[0] == 'A' || ans[0] == 'a') return ;
  ilo = (int)x;
  gf3gd.mch[0] = ilo;
  dspmkr(1);
  retic (&x, &y, ans);
  ...
  printf ("  selected gate channels %i...%i\n",ilo,ihi);
  for (i = ilo; i <= ihi; i++) {
    gf3gd.mch[0] = i;
    dspmkr(1);
  }
  ...
  printf("Set limits for background gates; 'A' to abort; 'X' or right-mouse when done...\n");
  bg = 0;
  totbgch = 0;
  while (bg<30) {
    retic (&x, &y, ans);
    ...
    bgilo[bg] = (int) x;
    gf3gd.mch[0] = bgilo[bg];
    dspmkr(1);    
    retic (&x, &y, ans);
    if (ans[0] == 'A' || ans[0] == 'a') return ;
    if (ans[0] == 'X' || ans[0] == 'x') break;
    bgihi[bg] = (int) x;
    gf3gd.mch[0] = bgihi[bg];
    dspmkr(1);    

    /* successfully entered both limits of bg, so finalize bg region */
    if (bgilo[bg] > bgihi [bg]) {
      SWAP(bgilo[bg],bgihi[bg]);
    }
    printf ("  selected background channels %i...%i\n",bgilo[bg],bgihi[bg]);
    totbgch += bgihi[bg] - bgilo[bg] + 1;
    for (i = bgilo[bg]; i <= bgihi[bg]; i++) {
      gf3gd.mch[0] = i;
      dspmkr(1);
    }
    bg++;
  ...
  numbg = bg;
  printf ("  selected %d background regions totaling %d channels\n", numbg, totbgch); 

  printf ("Performing gate on matrix file %s...\n", gf3gd.matfilnam);
  matread_slice(gf3gd.matfilnam,gf3gd.spec,&numch, ilo, ihi, gateaxis, 0, 1.0);
  weight = ((float) totgatech / totbgch );    
  for (bg = 0; bg < numbg; bg++) {
    /* Weight each background channel so that the "integral" of background over all background 
       regions equals integral of background in gate region.*/
    matread_slice(gf3gd.matfilnam,gf3gd.spec,&numch, bgilo[bg], bgihi[bg], gateaxis, 1, - weight);

  }

  gf3gd.maxch = numch - 1;
  strncpy(gf3gd.filnam,"--------",8);
  strncpy(gf3gd.namesp,"GATE    ",8);
  colormapsave = colormap[1];
  if (gf3gd.randk != 0)
    colormap[1] = 1;        /* enforce color scheme */
  gfexec("DS 1",4);

  /* calculate randoms subtraction */
  if (gf3gd.randk != 0)
    {
      /* validate spectra to be used */
      if ( (gf3gd.maxx != gf3gd.randsinglesmaxx) && (gf3gd.maxy != gf3gd.randsinglesmaxy) )
        {
          printf ("ERROR: Cannot perform randoms subtraction since matrix and singles\n");
          printf ("       dimensions do not agree.\n");
          printf ("       matrix %d x %d, singles %d x %d\n", gf3gd.maxx+1, gf3gd.maxy+1, 
                  gf3gd.randsinglesmaxx+1, gf3gd.randsinglesmaxy+1);
          return;
        }

      printf ("Performing randoms subtraction.  K = %e\n", gf3gd.randk);
        
      if (gateaxis == 'x')
        {
          /* calculate net number of singles counts in gate region minus background regions */
          gatesinglescts = 0.;
          for (ix = ilo; ix <= ihi; ix++)
            gatesinglescts += gf3gd.randsinglesx[ix];
          bgsinglescts = 0;
          for (bg = 0; bg < numbg; bg++) 
            for (ix = bgilo[bg]; ix <= bgihi[bg]; ix++)
              bgsinglescts += gf3gd.randsinglesx[ix];
          printf ("  singles counts: gate region %d, background regions %d, weighted net %.1f\n", 
                  gatesinglescts, bgsinglescts, (gatesinglescts - weight * bgsinglescts));
          randmultiplier = gf3gd.randk * (gatesinglescts - weight * bgsinglescts);
          printf ("  resulting multiplication constant for singles subtraction %f\n", randmultiplier);
            
          /* subtract randoms */
          for (iy = 0; iy <= gf3gd.maxy; iy++)
            gf3gd.spec[iy] -= randmultiplier * gf3gd.randsinglesy[iy];
        }
      else if (gateaxis == 'y')
        {
          /* calculate net number of singles counts in gate region minus background regions */
          gatesinglescts = 0.;
          for (iy = ilo; iy <= ihi; iy++)
            gatesinglescts += gf3gd.randsinglesy[iy];
          bgsinglescts = 0;
          for (bg = 0; bg < numbg; bg++) 
            for (iy = bgilo[bg]; iy <= bgihi[bg]; iy++)
              bgsinglescts += gf3gd.randsinglesy[iy];
          printf ("  singles counts: gate region %d, background regions %d, weighted net %.1f\n", 
                  gatesinglescts, bgsinglescts, (gatesinglescts - weight * bgsinglescts));
          randmultiplier = gf3gd.randk * (gatesinglescts - weight * bgsinglescts);
          printf ("  resulting multiplication constant for singles subtraction %f\n", randmultiplier);
          ...
          /* subtract randoms */
          for (ix = 0; ix <= gf3gd.maxy; ix++)
            gf3gd.spec[ix] -= randmultiplier * gf3gd.randsinglesx[ix];
        }
      /* overlay subtracted spectrum */
      gf3gd.maxch = numch - 1;
      strncpy(gf3gd.namesp,"GATESUB ",8);
        
      colormap[1] = 2;
      /* gfexec("CO 2",4); -- FAILS TO WORK ON SENCOND APPLICATION OF gate() */
      gfexec("OV 1",4);
        
      colormap[1] = colormapsave;
...
void limit_display()
  {
    if (gf3gd.numx > gf3gd.maxch + 1) {
      printf ("  max display channels %d\n", gf3gd.maxch);
      gf3gd.numx = gf3gd.maxch + 1;
    }
    if (gf3gd.lox < 0) {
      printf ("  lower display limit 0\n");
      gf3gd.lox = 0;
    }
    if (gf3gd.lox + gf3gd.numx - 1 > gf3gd.maxch) {
      printf ("  upper display limit %d\n", gf3gd.maxch);
      gf3gd.lox = gf3gd.maxch - gf3gd.numx + 1;
    }
  }

/* ======================================================================= */

int findsharedlohi (int maxspec, int in1, int in2, int in3)

     /* saves lo (<=0) and hi (>=0) counts of all displayed spectra 
        to common variables sharedlocnt and sharedhicnt

        History: Scratch file read/write is lifted directly from SHRDSP: in gfexec().
        Calculation of x limits if lifted from dspsp().
        Count search is modeled after dspsp().
     */
{
  int  i, in, numspec, idata = 0;
...
#define R(a,b,c) fread(b,c,a,scrfilea)
  for (numspec = 1; numspec <= maxspec; ++numspec) {
    ...
    /* not lifted from SHRDSP: */ 
    /* here sharedlocnt and sharedhicnt are found  */
    /* note: gf3gd.loch and gf3gd.nchs are x-axis limits calculated in dspsp() */
    for (i = gf3gd.loch; i <= gf3gd.loch + gf3gd.nchs - 1; ++i) {
      {
        if (gf3gd.sharedlocnt > gf3gd.spec[i]) gf3gd.sharedlocnt = gf3gd.spec[i];
        if (gf3gd.sharedhicnt < gf3gd.spec[i]) gf3gd.sharedhicnt = gf3gd.spec[i];
      }
...
int writeall(int *maxspec, char *ans, int nc)
{
  /* write spectrum in spec array to disk file
     default file extension = .spe */

  int idata = 0, in = 0;  /* dummies */
  int  iext;
  char fn[80];
  FILE *file;
  int buffer[128], numread;
  ...
  /* open output file */
  iext = setext(fn, ".gf3m", 80);
  if (!(file = open_new_file(fn, 0))) return 1;

  /* write current spectrum and its settings */
#define W(a,b,c) fwrite(b,c,a,file)
  W(15, colormap, 4);
  W(1, &gf3gd.locnt, 4);
  W(1, &gf3gd.ncnts, 4);
  W(1, &gf3gd.iyaxis, 4);
...
#undef W
  ...
  fwrite (maxspec,sizeof(int),1,file);

  /* write displayed spectra */
  fflush(scrfilea);
  rewind(scrfilea);

  while (!feof(scrfilea))
    {
      numread = fread (&buffer, sizeof(int), 128,scrfilea);
      if (fwrite (&buffer,sizeof(int),numread,file) != numread)
        goto WERR;
    }


  fclose(file);
  printf ("%d spectra written...\n", *maxspec);
  return 0;
...
int readall(int *maxspec, char *ans, int nc)
{
  /* write spectrum in spec array to disk file
     default file extension = .spe */
  ...
  if (nc < 3) {
    ...
    nc = cask(" Name of input file = ?", fn, 80);
    if (nc == 0) return 0;
  }
  ...
  /* read current spectrum and its settings */
#define R(a,b,c) fread(b,c,a,file)
  R(15, colormap, 4);
  R(1, &gf3gd.locnt, 4);
  R(1, &gf3gd.ncnts, 4);
  R(1, &gf3gd.iyaxis, 4);
...
#undef R
  ...
  /* read number of saved spectra */
  fread (maxspec,sizeof(int),1,file);
  ...
  rewind(scrfilea);
   while (!feof(file))
    {
      numread = fread (&buffer, sizeof(int), 128,file);
      if (fwrite (&buffer,sizeof(int),numread,scrfilea) != numread)
        goto RERR;
    }


  fclose(file);
  printf ("%d spectra read...\n", *maxspec);
  return 0;

 RERR:
  file_error("read from", fn);
  fclose(file);
  return 1;
} /* wrtsp */
...
void display_gf3m_help()
{
  ...
  printf(
    ...
    /* RadWare convention: x cycles most rapidly in file.  
       Corresponds to tscan argument ordering inc_xxxxx(x,y).
       This is the reverse of Scana's convention. */
}
/* ======================================================================= */

/* necessary prerequisites lifted from miniga.c */
#include <X11/Xlib.h>
...
int hcopy_noninteractive(char* ans)
     /* minor adaptation of hcopy() in libs/minig/miniga.c */

{
  ...
  char   s = 'm', c, dattim[20], text[80], /* mc ans[80],*/ fn[80] = "mhc.ps";
  FILE   *psfile;
  Window junk_id;
  ...
  extern int pr_and_del_file(char *);
  ...
  if (cask("   (default .ext = .ps)", ans, 80)) strcpy(fn, ans); */
  color = 1;
  filegiven = sscanf(ans+2,"%s",fn);
  /* if !filegiven then fn defaults to mhc.ps */
  setext(fn, ".ps", 80);
