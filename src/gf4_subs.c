...
struct {
  ...
  int    npars, nfp, freepars[111], npks, irelw, irelpos, ncommonpars;
...
int adddelpk(int mode, int npeak)
{
  ...
  if (mode == 1) {
    ...
    y = gf3gd.pars[0] + gf3gd.pars[1] * x + gf3gd.pars[2] * x * x;  // gf3
    // y = 0;  // gf4
	...
	if (gf3gd.cents[i] > gf3gd.cents[j]) {
	  ...
	  gf3gd.dcents[j] = s;
	  for (k = i * 3 + gf3gd.ncommonpars; k < i * 3 + gf3gd.ncommonpars + 3; ++k) {
	    l = (j-i) * 3 + k;
	    s = gf3gd.pars[k];
...
int autofit(float *sens, int lo_ch, int hi_ch)
{
  ...
  NEXTFIT:
    dspmkr(99);
    ready = 1;
    gf3gd.npars = gf3gd.ncommonpars + gf3gd.npks * 3;
    ...
    for (i = 1; i <= gf3gd.npks; ++i) {
      if (gf3gd.pars[i*3 + gf3gd.ncommonpars-1] < 0.f) {
	if (!first) {
	  printf("One of the peaks (at %.2f -> %.2f) went negative, ending autofit...\n",
		 gf3gd.ppos[i-1], gf3gd.pars[i*3 + gf3gd.ncommonpars-3]);
	  quit_next = 1;
	}
	...
	goto NEXTFIT;
      ...
      if (gf3gd.pars[i*3 + gf3gd.ncommonpars-3] < (float) gf3gd.mch[0] ||
	  gf3gd.pars[i*3 + gf3gd.ncommonpars-3] > (float) gf3gd.mch[1]) {
    ...
    if (gf3gd.npks == 33 || quit_next) break;
    first = 0;
...
int dspfit(void)
{
  ...
  if (gf3gd.npks > 1) {
    ...
    for (i = 3; i < gf3gd.ncommonpars; ++i) {
      b[i] = gf3gd.pars[i];
    }
    b[5] = 0.f;
    for (j = 0; j < gf3gd.npks; ++j) {
      for (i = gf3gd.ncommonpars; i < gf3gd.ncommonpars+3; ++i) {
	b[i] = gf3gd.pars[j*3 + i];
      }
      eval(b, 0, &y, 1, -9);
      ilo = b[gf3gd.ncommonpars] - b[gf3gd.ncommonpars+1] * 3.f;
      ihi = b[gf3gd.ncommonpars] + b[gf3gd.ncommonpars+1] * 3.f;
      if (b[3] > 0.01 && ilo > b[gf3gd.ncommonpars] - b[4] * 6.f) ilo = b[gf3gd.ncommonpars] - b[4] * 6.f;
      if (ilo < lo) ilo = lo;
      if (ihi > hi) ihi = hi;
      eval(b, ilo, &y, 1, 0);
      x1 = (float) ilo + .5f;
...
int eval(float *pars, int ichan, float *fit, int npks, int mode)
{
  ...
  float a, h, r, r1, r2, u, u1, u2, u3, u5, u6, u7, u8, w, x, x1, z, x3; 
  float rb, betab, zb, r2b, u2b, u5b, u6b, u7b, u8b, hh; 
  static float y[35], y1[35], y2[35], yb[35], y1b[35], y2b[35], beta, width;
  static int   i, notail, notailb, ix0;
  ...
  if (mode == -9) {
    ...
    notail = notailb = 0;
    for (i = 0; i < npks; ++i) {
      y[i]  = pars[i*3 + gf3gd.ncommonpars+1] / (pars[4] * 3.33021838f);
      yb[i] = pars[i*3 + gf3gd.ncommonpars+1] / (pars[7] * 3.33021838f);
      if (y[i] > 20.f) {
	y1[i] = 0.f;
	notail = 1;
      } else {
	...
	y1b[i] = erfc(yb[i]);
 	y2b[i] = exp(-yb[i] * yb[i]) * 1.12837917f / y1b[i];
      }
    }
    return 0;
  }

  // u1 = gaussian, u2, u2b = tails, u3 = backscatter (step in gf3)
  x = (float) (ichan - ix0);
  *fit = pars[0] + pars[1]*x + pars[2]*x*x;
  ...
  x1 = (float) (ichan);
  for (i = 0; i < npks; ++i) {
    x = x1 - pars[i*3 + gf3gd.ncommonpars];
    width  = pars[i*3 + gf3gd.ncommonpars+1] / 2.35482f;
    h      = pars[i*3 + gf3gd.ncommonpars+2];
    ...
    u3 = 0.f;
    // x3 = pars[8] - x/pars[i*3 + gf3gd.ncommonpars];
    x3 = 1.0 - (float) (ichan) / (pars[i*3 + gf3gd.ncommonpars] * (1.0+pars[8]));
    if (x3 > 0.0 && x3 < 1.0) {
      u3 = pow(x3, pars[9]) * pow(1.0-x3, pars[10]) / 100.0;
    }
    if (mode == -1) {
	...
	derivs[5] += h * u3;
        if (x3 > 0.0 && x3 < 1.0) {
          hh = -(h * pars[5] / 100.0) * (1.0-x3) / (1.0+pars[8]);
          derivs[8] += hh * pars[9]*pow(x3, pars[9]-1.0) * pow(1.0-x3, pars[10]);
          derivs[8] -= hh * pow(x3, pars[9]) *  pars[10]*pow(1.0-x3, pars[10]-1.0);
          derivs[9] += h * pars[5] * log(x3)*pow(x3, pars[9]) * pow(1.0-x3, pars[10]) / 100.0;
          derivs[10] += h * pars[5] * pow(x3, pars[9]) * log(1.0-x3)*pow(1.0-x3, pars[10]) / 100.0;
        }
	a = u1 * w * 2.f;
	derivs[i*3 + gf3gd.ncommonpars  ] = h * a / (width * 1.41421356f);
	derivs[i*3 + gf3gd.ncommonpars+1] = h * w * a * 1.41421356f / (width * 2.35482f);
	derivs[i*3 + gf3gd.ncommonpars+2] = u;
      }
      continue;
    }
    r  = pars[3] / 100.f;
    ...
    betab = pars[7];
    zb = w + yb[i];
    if ((r2b = x / betab, fabs(r2b)) > 60.f) {
      u5b = 0.f;
      u6b = 0.f;
      u7b = 0.f;
    } else {
      u7b = exp(x / betab) / y1b[i];
      if (fabs(zb) > 20.f) {
	u5b = 0.f;
	if (zb < 0.f) u5b = 2.f;
	u6b = 0.f;
	...
	u5b = erfc(zb);
	u6b = exp(-zb * zb) * 1.12837917f;
      }
    }
    u2b = u7b * u5b;

    // u1 = gaussian, u2 = tail, u3 = backscatter (step in gf3)
    u = r1 * u1 + r * u2 + rb * u2b + pars[5] * u3;
    *fit += h * u;
    /* calculate derivs only for mode.ge.1 */
    if (mode >= 1) {
      u8  = u5  * y2[i];
      ...
      derivs[5] += h * u3;
      derivs[6] += h * (u2b - u1) / 100.f;
      derivs[7] += rb * h * u7b * (yb[i] * (u6b - u8b) - u5b * x / betab) / betab;

      //x3 = pars[8] - x/pars[i*3 + gf3gd.ncommonpars]);
      if (x3 > 0.0 && x3 < 1.0) {
        //u3 = pow(x3, pars[9]) * pow(1.0-x3, pars[10]) / 100.0;
        derivs[8] += h * pars[5] * pars[9]*pow(x3, pars[9]-1) * pow(1.0-x3, pars[10]) / 100.0;
        derivs[8] -= h * pars[5] * pow(x3, pars[9]) *  pars[10]*pow(1.0-x3, pars[10]-1) / 100.0;
        derivs[9] += h * pars[5] * log(x3)*pow(x3, pars[9]) * pow(1.0-x3, pars[10]) / 100.0;
        derivs[10] += h * pars[5] * pow(x3, pars[9]) * log(1.0-x3)*pow(1.0-x3, pars[10]) / 100.0;
      }

      a = u1 * r1 * w * 2.f;
      derivs[i*3 + gf3gd.ncommonpars  ] = h * (a + r*u7*(u6 - u5*2.f*y[i])) / (width*1.41421356f);
      derivs[i*3 + gf3gd.ncommonpars+1] = h * (w*a +
                                               r*u7*(u6 * (w - y[i]) + u8*y[i]) +
                                               rb*u7b*(u6b * (w - yb[i]) + u8b*yb[i])) *
...
int fitter(int maxits)
{
  ...
  /* nip   = no. of independent (non-fixed) pars
     ...
     npars = total no. of pars = 3 * no.of peaks + ncommonpars (= 6 for gf3, 11 for gf4)
     ndf   = no. of degrees of freedom */
  if (gf3gd.wtmode > 0) strncpy(&wtc[2][4], gf3gd.nwtsp, 8);
  for (i = gf3gd.ncommonpars; i < gf3gd.npars; ++i) {
  ...
  if (gf3gd.irelw <= 0) {
    niw = 0;
    for (j = gf3gd.ncommonpars+1; j < gf3gd.npars; j += 3) {
    ...
    if (niw > 1) {
      for (j = gf3gd.ncommonpars+1; j <= miw; j += 3) {
	gf3gd.freepars[j] = 0;
      }
      rwfixed = 1;
      nip = nip - niw + 1;
  ...
  if (gf3gd.irelpos <= 0) {
    nipos = 0;
    for (j = gf3gd.ncommonpars; j < gf3gd.npars; j += 3) {
    ...
    if (nipos > 1) {
      for (j = gf3gd.ncommonpars; j <= mip; j += 3) {
	gf3gd.freepars[j] = 0;
      }
      rpfixed = 1;
      nip = nip - nipos + 1;
 ...
 NEXT_ITERATION:
  ...
  for (i = ilo; i <= ihi; ++i) {
    ...
    if (rwfixed) {
      for (k = gf3gd.ncommonpars+1; k < miw; k += 3) {
	derivs[miw] += fixed[k] * derivs[k];
      }
    }
    if (rpfixed) {
      for (k = gf3gd.ncommonpars; k < mip; k += 3) {
 ...
 INVERT_MATRIX:
  ...
  if (!test) {
    ...
    if (rwfixed) {
      for (j = gf3gd.ncommonpars+1; j <= miw - 3; j += 3) {
	b[j] = gf3gd.pars[j] + fixed[j] * delta[nip1-1];
      }
    }
    if (rpfixed) {
      for (j = gf3gd.ncommonpars; j <= mip - 3; j += 3) {
...
int fix_para(int param, int fix_flag)
{
  ...
  if (fix_flag) {
	...
	} else if (gf3gd.nfp + gf3gd.freepars[param] == gf3gd.npars - 1) {
	  printf("  Cannot - too many fixed pars.\n");
	  continue;
	} else {
	  gf3gd.nfp += gf3gd.freepars[param];
	  gf3gd.freepars[param] = 0;
	  printf(" >>> Beta or BB fixed at %.3f\n", gf3gd.pars[param]);
	  gf3gd.pars[param-1] = val;
	}
	break;
      }
...
int fixorfree(char *command, int nc)
{
  ...
  char fixtag[118][4], ans[80];
  static char parc[118][4] = {
    " A "," B "," C "," R ","BTA","STP","RR ","BB "," X "," Y "," Z ",
    "P1 ","W1 ","H1 ","P2 ","W2 ","H2 ","P3 ","W3 ","H3 ","P4 ","W4 ","H4 ",
    "P5 ","W5 ","H5 ","P6 ","W6 ","H6 ","P7 ","W7 ","H7 ","P8 ","W8 ","H8 ",
    "P9 ","W9 ","H9 ","PA ","WA ","HA ","PB ","WB ","HB ","PC ","WC ","HC ",
    "PD ","WD ","HD ","PE ","WE ","HE ","PF ","WF ","HF ","PG ","WG ","HG ",
    "PH ","WH ","HH ","PI ","WI ","HI ","PJ ","WJ ","HJ ","PK ","WK ","HK ",
    "PL ","WL ","HL ","PM ","WM ","HM ","PN ","WN ","HN ","PO ","WO ","HO ",
    "PP ","WP ","HP ","PQ ","WQ ","HQ ","PR ","WR ","HR ","PS ","WS ","HS ",
    "PT ","WT ","HT ","PU ","WU ","HU ","PV ","WV ","HV ","PW ","WW ","HW ",
    "PX ","WX ","HX ","PY ","WY ","HY ","PZ ","WZ ","HZ ","RW ","RP "};
  ...
  strcpy(fixtag[111], "   ");
  if (!gf3gd.irelw) strcpy(fixtag[116], "  *");
  strcpy(fixtag[112], "   ");
  if (!gf3gd.irelpos) strcpy(fixtag[117], "  *");
  ...
  printf(" %s %s", fixtag[116], fixtag[117]);
  printf("\n ");
  for (i = lo; i < gf3gd.npars; ++i) {
    printf(" %s", parc[i]);
  }
  printf(" %s %s", parc[116], parc[117]);
...
int gfexec(char *ans, int nc)
{
  ...
  float sens, x, y, x1, x2, e1, e2, save[16384], save2[16384], sum;
  ...
  if (!strncmp(ans, "AC", 2) || !strncmp(ans, "AS", 2) || 
    ...
    for (i=0; i < numch; i++) {
      for (j=0; i+j < numch; j++) {
        save[i+j] += gf3gd.spec[i]*gf3gd.spec[j];  // do convolution
      }
    }
    for (i = 0; i < numch; ++i) gf3gd.spec[i] = 0.5 * save[i] / sum;
  } else if (!strncmp(ans, "CN", 3)) {
    /* convolve with another (new) spectrum */
    numch = gf3gd.maxch + 1;
    sum = 0;
    for (i = 0; i < numch; ++i) {
      save[i] = 0;
      save2[i] = gf3gd.spec[i];
      sum += gf3gd.spec[i];
    }
    if (getsp(ans, nc)) return 0;
    for (i=0; i < numch; i++) {
      for (j=0; i+j < numch; j++) {
        save[i+j] += save2[i]*gf3gd.spec[j];  // do convolution
      }
    }
    for (i = 0; i < numch; ++i) gf3gd.spec[i] = 0.5 * save[i] / sum;
...
#undef W
    ...
    char  l[120];
    if (gf3gd.ical) {
      printf("Peak  channel       centroid      energy        FWHM[chs]    FWHM[keV]\n");
    } else {
      printf("Peak  channel       centroid      FWHM[chs]\n");
    }
    ...
    for (j = 0; j < gf3gd.npks; ++j) {
      for (i = gf3gd.ncommonpars; i < gf3gd.ncommonpars+3; ++i) b[i] = gf3gd.pars[j*3 + i];
      b[gf3gd.ncommonpars+1] *= 20.0f;   // increase width of peak by factor 20
      ...
      *l= '\0';
      ...
      wrresult(l, fw, gf3gd.errs[k+1], 55);
  ...
  } else if (!strncmp(ans, "SF", 2)) {   /* store fit in spectrum, or subtract it */
    if (gf3gd.npks > 0) {
      if (!caskyn("Subtract fit from spectrum instead of storing it? (Y/N)")) {
        eval(gf3gd.pars, 0, &y, gf3gd.npks, -9);
        for (i=0; i <= gf3gd.maxch; i++) {
          eval(gf3gd.pars, i, &y, gf3gd.npks, 0);
          gf3gd.spec[i] = y;
    ...
    if (nc > 2 && (ispd=1) &&
	inin(ans + 2, nc - 2, &isplo, &isphi, &ispd)) goto BADCMD;
...
#endif
    //gf3gd.ncommonpars = 6;  // gf3
    gf3gd.ncommonpars = 11;  // gf4
...
int parset(int mode)
{
  ...
  if (gf3gd.freepars[0] == 1)
    // gf3gd.pars[0] = (gf3gd.spec[ihi] + gf3gd.spec[ilo]) / 2.f; // gf3
    gf3gd.pars[0] = 0; // gf4
  if (gf3gd.freepars[1] == 1)
    //gf3gd.pars[1] = (gf3gd.spec[ihi] - gf3gd.spec[ilo]) / (float) (ihi-ilo); // gf3
    gf3gd.pars[1] = 0; // gf4
  ...
  if (gf3gd.freepars[6] == 1) gf3gd.pars[6] = gf3gd.pars[3]*0.07;
  if (gf3gd.freepars[7] == 1) gf3gd.pars[7] = gf3gd.pars[4]*5.0;
  if (gf3gd.freepars[8] == 1) gf3gd.pars[8] = 0.02;
  if (gf3gd.freepars[9] == 1) gf3gd.pars[9] = 1.7;
  if (gf3gd.freepars[10] == 1) gf3gd.pars[10] = 0.5;
  if (mode < 0) {
    /* come here only during set-up */
    resetp = 1;
    resetw = 1;
    gf3gd.nfp = 11 - 2*gf3gd.infix[0] - 2*gf3gd.infix[1] - gf3gd.infix[2];
    gf3gd.freepars[0] = gf3gd.freepars[1] = gf3gd.freepars[2] = 0; // gf4
    gf3gd.freepars[8] = gf3gd.freepars[9] = gf3gd.freepars[10] = 0; // gf4
    gf3gd.freepars[3] = gf3gd.freepars[6] = gf3gd.infix[0];
    gf3gd.freepars[4] = gf3gd.freepars[7] = gf3gd.infix[1];
    gf3gd.freepars[5] = gf3gd.infix[2];
    if (gf3gd.freepars[3] == 0 && gf3gd.pars[3] == 0.f) {
      gf3gd.nfp += gf3gd.freepars[4];
      gf3gd.freepars[4] = 0;
    }
    if (gf3gd.freepars[6] == 0 && gf3gd.pars[6] == 0.f) {
      gf3gd.nfp += gf3gd.freepars[7];
      gf3gd.freepars[7] = 0;
    }
    gf3gd.irelw = gf3gd.infixrw;
    if (gf3gd.infixw == 0) {
      for (i = 1; i <= gf3gd.npks; ++i) {
	gf3gd.freepars[i*3 + gf3gd.ncommonpars-2] = 0;
      }
...
int sumcts_jch(void)
{
  ...
  while (1) {
    ...
    npks2 = 0;
    for (j = 0; j < gf3gd.npks; j++) {
      if (gf3gd.pars[3*j+gf3gd.ncommonpars] > loch &&
          gf3gd.pars[3*j+gf3gd.ncommonpars] < hich) {
	for (i = gf3gd.ncommonpars; i < gf3gd.ncommonpars+3; ++i) {
...
int list_params(int mode)
{
  ...
  strcpy(l, "      Shape: RR =");
  wrresult(l, gf3gd.pars[6], gf3gd.errs[6], 0);
  strcat(l, ",  BB =");
  wrresult(l, gf3gd.pars[7], gf3gd.errs[7], 0);
  printf("%s\n", l);
  if (prfile) fprintf(prfile, "%s\n", l);

  strcpy(l, "      Shape: X =");
  wrresult(l, gf3gd.pars[8], gf3gd.errs[8], 0);
  strcat(l, ",  Y =");
  wrresult(l, gf3gd.pars[9], gf3gd.errs[9], 0);
  strcat(l, ",  Z =");
  wrresult(l, gf3gd.pars[10], gf3gd.errs[10], 0);
  printf("%s\n", l);
  if (prfile) fprintf(prfile, "%s\n", l);
