...
int calc_peaks(void)
{
  ...
  float r1, u1, u7, u5, u6, eg, dx, deg, pos, y = 0.0f, y1 = 1.0f;
...
int get_bkgnd(void)
{
  ...
  float bsp4k[10][4096], fsum1, fsum2, fsum3, f, fj1, fj2, factor1, factor2;
  ...
  if (!askyn("Use enhanced background from E2-bump gates? (Y/N)")) {
	...
	if (read_spe_file(filnam, bsp4k[i], namesp, &nch, MAXCHS)) continue;
	if (nch == 4096) break;
    ...
    for (i = 0; i < ihich; ++i) {
      fsum1 += bsp4k[0][i];
      bsp4k[6][i] = bsp4k[0][i] - bsp4k[2][i];
    }
    for (j = 0; j < ihich; ++j) {
      bsp4k[7][j] = bsp4k[8][j] = bsp4k[9][j] = 0.0f;
      bsp4k[2][j] /= fsum1;
    }
	...
	if (!(k = ask(ans, 80, " Factor 1 = ? (rtn for %f)", factor1)) ||
      ...
      for (j = 0; j < ihich; ++j) {
	bsp4k[7][j] = bsp4k[1][j] - bsp4k[0][j] / factor1;
	bsp4k[8][j] = bsp4k[3][j] - bsp4k[2][j] / factor1;
	bsp4k[9][j] = bsp4k[7][j] - bsp4k[8][j];
	bsp4k[2][j] /= fsum1;
      }
      while (1) {
  ...
  if (xxgd.esc_file_flags[0] != 1) {
    for (i = 0; i < xxgd.numchs; ++i) {
      xxgd.bspec[0][i] = bsp4k[0][2*i] + bsp4k[0][2*i + 1];
      xxgd.bspec[1][i] = bsp4k[2][2*i] + bsp4k[2][2*i + 1];
      xxgd.bspec[2][i] = bsp4k[6][2*i] + bsp4k[6][2*i + 1];
      xxgd.bspec[3][i] = bsp4k[7][2*i] + bsp4k[7][2*i + 1];
      xxgd.bspec[4][i] = bsp4k[8][2*i] + bsp4k[8][2*i + 1];
      xxgd.bspec[5][i] = bsp4k[9][2*i] + bsp4k[9][2*i + 1];
    }
  } else {
    for (i = 0; i < xxgd.numchs; ++i) {
      xxgd.bspec[0][i] = bsp4k[0][i];
      xxgd.bspec[1][i] = bsp4k[2][i];
      xxgd.bspec[2][i] = bsp4k[6][i];
      xxgd.bspec[3][i] = bsp4k[7][i];
      xxgd.bspec[4][i] = bsp4k[8][i];
      xxgd.bspec[5][i] = bsp4k[9][i];
    }
  }
  while ((nc = ask(ans, 80,
...
int get_gate(int ilo, int ihi)
{
  ...
  for (iy = ilo; iy <= ihi; ++iy) {
    fseek(filez[2], iy*reclen, SEEK_SET);
    fread(xxgd.rdata, xxgd.numchs, 4, filez[2]);
    for (ix = 0; ix < xxgd.numchs; ++ix) {
      xxgd.spec[0][ix] += xxgd.rdata[ix];
    }
    if (xxgd.esc_file_flags[2]) {
		      ...
		      float *wfact)
{
  ...
  *outnum = 0;
  ...
  /* list name not found */
  if (ffin(c, nc, &eg, wfact, &fj2)) return 1;
  *outnum = 1;
  outlist[0] = eg;
  if (*wfact <= 0.0f) *wfact = 1.0f;
  return 0;
...
int prep(void)
{
  ...
  int   jext, i, j, iymax, i0, i4, ii, iy, ix, m4b;
  int   matseg4b[4096*8];
  short matseg[4096*8];
  ...
  warn1(
    ...
    "   .spn or .m4b (4B/ch) formats.\n"
    " Specify .ext = .spn or .m4b for 4B/ch.\n\n");
  ...
  if (xxgd.esc_file_flags[0] != 1) {
    ...
    if (xxgd.numchs == 4096) {
      fprintf(filez[26], "      ...by taking all 4k channels.\n");
  ...
  ii = iymax;
  fmax = 1e-6f;
  fseek(filez[2], ii*reclen, SEEK_SET);
  fread(xxgd.spec[0], 4, xxgd.numchs, filez[2]);
