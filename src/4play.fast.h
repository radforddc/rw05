...
#define RW_MAXCH 1024            /* Maximum channels on cube axis */
                                 /*    (must be multiple of 4)    */

/* a = RW_MAXCH/4                 = 256
   b = a*(a+1)*(a+2)*(a+3)/24     = 183181376
   c = ceil(b/256)                = 715553
   RW_LB2 = ceil(c/RW_CHNK_S)     = 3727
   RW_LB1 = RW_LB2*RW_CHNK_S      = 715584

   RW_DB1 = floor((RW_SCR_RECL-4)/(2*(RW_DB2+1)))
   */

#define RW_LB1 715776      /* length required for buf1, depends on RW_MAXCH */
                           /* RW_LB1 > x(x+1)(x+2)(x+3)/(24*256),           */
		           /* where x = RW_MAXCH/4                          */
                           /* must also be a multiple of RW_CHNK_S          */
#define RW_DB1 20           /* depth of buf1 */

#define RW_LB2 1398        /* length required for buf2, depends on RW_LB1   */
                           /* RW_LB2 >= RW_LB1/RW_CHNK_S                    */
#define RW_DB2 780         /* depth of buf2 not to exceed RW_SCR_RECL       */
                           /* byte records in scr file                      */

#define RW_SCR_RECL 32768  /* Record length of scr file                     */
                           /* must be >= RW_DB2*2*(RW_DB1+1) + 4            */
#define RW_CHNK_S  512     /* size of update chunk, in units of 1/8 MB      */
