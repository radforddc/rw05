...
int gfexec(char *ans, int nc)
{
  ...
  char  q[256], cmd[80];
  static int isplo = 1, isphi = 2, ispd, ssnum = 0, maxspec = 0, last_snum = -1;
...
#undef W
    ...
    for (i=idata; i<=in; i++) gf3gd.spec[i] -= x1 + (float) i * y;
    for (i=0; i<idata; i++) gf3gd.spec[i] = 0;
    for (i=in+1; i<=gf3gd.maxch; i++) gf3gd.spec[i] = 0;
    ...
    if (ispd >= 0) erase();  // OS lo hi -1 to NOT clear
    if (ispd < 1) ispd = 1;
    j = colormap[1];
  ...
  } else if (!strncmp(ans, "II", 2)) {
    if (inin(ans + 2, nc - 2, &idata, &in, &in2)) {
      idata = 1; in = gf3gd.maxch;
    }
    if (idata < 1) idata = 1;
    if (in > gf3gd.maxch) in = gf3gd.maxch;
    for (i = 0; i < idata; i++) gf3gd.spec[i] = 0;
    for (i=idata; i <= in; i++) {
      gf3gd.spec[i] += gf3gd.spec[i-1];
    }
